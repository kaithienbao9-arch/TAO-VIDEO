...
export function playTypewriterClick(audioContext: AudioContext, destinationNode?: AudioNode, volume = 100) {
  ...
  try {
    ...
    // Boosted slightly from 0.012 to 0.04 for high audibility
    gainNode.gain.setValueAtTime(0.04 * volFactor, time);
    ...
    thudGain.gain.setValueAtTime(0.025 * volFactor, time);
  ...
  try {
    ...
    if (type === 'dog_bark') {
      ...
      const playBark = (delay: number) => {
        ...
        gainNode.gain.setValueAtTime(0.12 * volFactor, time + delay);
        ...
        // Add noise burst for bark texture
        ...
        noiseGain.gain.setValueAtTime(0.06 * volFactor, time + delay);
      ...
      latchGain.gain.setValueAtTime(0.06 * volFactor, time);
      ...
      const noiseGain = audioContext.createGain();
      noiseGain.gain.setValueAtTime(0.2 * volFactor, time + 0.01);
      ...
      subGain.gain.setValueAtTime(0.22 * volFactor, time + 0.01);
      ...
      gainNode.gain.linearRampToValueAtTime(0.1 * volFactor, time + 0.5);
      gainNode.gain.linearRampToValueAtTime(0.06 * volFactor, time + 1.3);
      ...
      const playHornElement = (freq: number, start: number, dur: number) => {
        ...
        gainNode.gain.linearRampToValueAtTime(0.06 * volFactor, start + 0.03);
        gainNode.gain.setValueAtTime(0.06 * volFactor, start + dur - 0.05);
      ...
      gainNode.gain.linearRampToValueAtTime(0.05 * volFactor, time + 0.3);
      gainNode.gain.linearRampToValueAtTime(0.04 * volFactor, time + 0.8);
      ...
      gainNode.gain.setValueAtTime(0.08 * volFactor, time);
      ...
      const playChalk = (delay: number) => {
        ...
        gainNode.gain.setValueAtTime(0.035 * volFactor, time + delay);
      ...
      const gainNode = audioContext.createGain();
      gainNode.gain.setValueAtTime(0.0001, time);
      gainNode.gain.linearRampToValueAtTime(0.07 * volFactor, time + 0.15);
      gainNode.gain.linearRampToValueAtTime(0.03 * volFactor, time + 0.35);
      gainNode.gain.exponentialRampToValueAtTime(0.0001, time + duration);
      ...
      const playCoughPulse = (delay: number, duration: number, volMultiplier: number) => {
        ...
        oscGain.gain.setValueAtTime(0.02 * volFactor * volMultiplier, time + delay);
        oscGain.gain.exponentialRampToValueAtTime(0.0001, time + delay + duration);
        
        const gainNode = audioContext.createGain();
        gainNode.gain.setValueAtTime(0.1 * volFactor * volMultiplier, time + delay);
      ...
      const playRing = (delay: number, duration: number) => {
        ...
        gainNode.gain.linearRampToValueAtTime(0.05 * volFactor, time + delay + 0.05);
        gainNode.gain.setValueAtTime(0.05 * volFactor, time + delay + duration - 0.05);
        gainNode.gain.exponentialRampToValueAtTime(0.0001, time + delay + duration);
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const clVol = 0.025 + Math.random() * 0.045;
