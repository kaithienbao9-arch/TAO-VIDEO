...
const PIPER_VOICES: PiperVoice[] = [
  {
    ...
    gender: 'Nam',
    accent: 'Miền Bắc',
    modelName: 'vi_VN-vits-lãng_khách-medium.onnx',
    onnxUrl: 'https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/vi/vi_VN/vits/medium/vi_VN-vits-l%C3%A3ng_kh%C3%A1ch-medium.onnx',
    jsonUrl: 'https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/vi/vi_VN/vits/medium/vi_VN-vits-l%C3%A3ng_kh%C3%A1ch-medium.onnx.json',
    description: 'Giọng đọc nam miền Bắc trầm ấm, dõng dạc, thích hợp cho video lịch sử, triết lý hoặc tin tức trang nghiêm.'
  },
  {
    id: 'hoai_nhac',
    name: 'Hoài Nhạc',
    gender: 'Nữ',
    accent: 'Miền Bắc',
    modelName: 'vi_VN-vits-hoài_nhạc-medium.onnx',
    onnxUrl: 'https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/vi/vi_VN/vits/medium/vi_VN-vits-ho%C3%A0i_nh%E1%BA%A1c-medium.onnx',
    jsonUrl: 'https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/vi/vi_VN/vits/medium/vi_VN-vits-ho%C3%A0i_nh%E1%BA%A1c-medium.onnx.json',
    description: 'Giọng đọc nữ miền Bắc trong trẻo, truyền cảm và truyền thống, rất phù hợp cho video review phim, kể chuyện, tâm sự.'
  }
];

const DEFAULT_SAMPLE_TEXT = `Chào mừng quý vị đã quay trở lại với hành trình khám phá ngày hôm nay.
Trong video này chúng ta sẽ cùng nhau phân tích những bí ẩn lịch sử chưa được giải đáp.
Mỗi chi tiết sẽ được tái hiện một cách sống động và trực quan nhất.
Đừng quên bấm đăng ký kênh để ủng hộ đội ngũ sản xuất nhé.`;
...
:: Tải giọng đọc Tiếng Việt được chỉ định nếu chưa tồn tại cục bộ
if not exist "${selectedVoice.modelName}" (
    echo [*] Đang tải về giọng đọc thông minh Tiếng Việt: ${selectedVoice.name} (${selectedVoice.gender} - ${selectedVoice.accent})...
...
Mỗi dòng viết xuống sẽ trở thành 1 mốc phụ đề hoàn hảo khớp với âm thanh phát ra!"
            ...
            <div className="space-y-4 border-t border-white/5 pt-4">
              ...
              <div className="space-y-2">
                <label className="text-[10px] text-white/40 uppercase block font-semibold">Giọng đọc Tiếng Việt</label>
