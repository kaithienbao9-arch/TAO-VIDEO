    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed)) {
          // Filter out default pre-saved effects
          return parsed.filter(eff => eff.id !== 'effect_default' && eff.id !== 'effect_cinematic');
        }
      } catch (e) {
          ...
          {activeTab === 'display' && (
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-white/50 font-medium block mb-1.5">Chiều ngang xuất (Width):</label>
                  <select
                    value={config.width}
                    onChange={(e) => updateConfig('width', parseInt(e.target.value))}
                    className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500"
                  >
                    <option value={1280}>1280 px (Khuyên dùng)</option>
                    <option value={1920}>1920 px (Full HD)</option>
                    <option value={640}>640 px (Chân dung vuông)</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs text-white/50 font-medium block mb-1.5">Chiều dọc xuất (Height):</label>
                  <select
                    value={config.height}
                    onChange={(e) => updateConfig('height', parseInt(e.target.value))}
                    className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500"
                  >
                    <option value={720}>720 px (Khuyên dùng)</option>
                    <option value={1080}>1080 px (Full HD)</option>
                    <option value={720}>720 px (Vuông tỉ lệ)</option>
                  </select>
                </div>
              </div>
...
{(config.enableTextHighlight || config.subtitleShowEffect === 'highlight_two_words') && (
                    ...
                    <div className="space-y-2 pt-2 border-t border-white/5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {savedEffects.filter(eff => (eff.group || 'A') === selectedEffectMẫu).map((eff) => (
                          <div 
                            key={eff.id}
                            className="bg-[#050505] border border-white/10 hover:border-blue-500/40 rounded-lg p-2.5 flex items-center justify-between gap-1 transition-all group"
                          >
                            <button
                              type="button"
                              onClick={() => {
                                // Open the fully-featured popup editing editor copy!
                                setPopupEditingEffect({
                                  ...eff,
                                  subtitleAlign: eff.subtitleAlign || 'center',
                                  blurBgX: eff.blurBgX !== undefined ? eff.blurBgX : eff.subtitleX,
                                  blurBgY: eff.blurBgY !== undefined ? eff.blurBgY : eff.subtitleY,
                                  blurBgShape: eff.blurBgShape || 'rectangle',
                                  blurBgColorHex: eff.blurBgColorHex || '#000000',
                                  blurBgBorderColorHex: eff.blurBgBorderColorHex || 'none',
                                  blurBgBlurAmount: eff.blurBgBlurAmount !== undefined ? eff.blurBgBlurAmount : 18,
                                  lockTextInBlur: !!eff.lockTextInBlur,
                                });
                              }}
                              className="flex-1 text-left min-w-0"
                            >
                              <h5 className="text-[11px] font-bold text-white/95 group-hover:text-blue-400 truncate">{eff.name}</h5>
                          ...
                          <div className="space-y-4 animate-in fade-in duration-200">
                            ...
                            <div className="space-y-2 pt-2.5 border-t border-white/5">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-mono text-amber-400 font-bold">
                                  {config.typewriterVolume !== undefined ? config.typewriterVolume : 100}%
                                </span>
                              </div>
                              <div className="flex items-center gap-3">
                                <input
                                  ...
                                  max="200"
                                  step="10"
                                  value={config.typewriterVolume !== undefined ? config.typewriterVolume : 100}
                                  ...
                                  onClick={() => {
                                    try {
                                      ...
                                      if (AudioContextClass) {
                                        ...
                                        playTypewriterClick(tempCtx, undefined, config.typewriterVolume !== undefined ? config.typewriterVolume : 100);
                                        setTimeout(() => {
                                          tempCtx.close().catch(() => {});
                                        }, 400);
                                      }
      ...
      {popupEditingEffect && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-[#0b0b0f] border border-white/10 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            
            {/* Header */}
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#121217]">
              <div className="flex items-center gap-2">
                <Sparkles className="text-blue-400 animate-pulse" size={16} />
                <h3 className="text-sm font-bold text-white">Chỉnh sửa Mẫu Hiệu ứng: <span className="text-blue-400">{popupEditingEffect.name}</span></h3>
              </div>
              <button 
                type="button" 
            ...
            <div className="p-5 space-y-5 overflow-y-auto flex-1 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-[11px] text-white/55 block mb-1.5 font-bold uppercase tracking-wider">Tên mẫu thiết kế hiệu ứng:</label>
                  <input
                    type="text"
                    value={popupEditingEffect.name}
                    onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, name: e.target.value }))}
                    placeholder="Ví dụ: Phong cách TikTok..."
                    className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500 font-medium"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-white/55 block mb-1.5 font-bold uppercase tracking-wider">Phân nhóm Mẫu chữ:</label>
                  <select
                    value={popupEditingEffect.group || 'A'}
                    onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, group: e.target.value }))}
                    className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500 font-medium cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
                {/* Entrance movement */}
                <div>
                  <label className="text-[11px] text-white/55 block mb-1.5 font-bold uppercase tracking-wider">Hiệu ứng vào (Entrance):</label>
                  <select
                    ...
                    className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-2.5 py-1.5 text-white focus:outline-none focus:border-blue-500"
                  >
                    <option value="zoom_fade">Thu phóng mờ dần (Zoom & Fade)</option>
                    <option value="bounce">Đàn hồi nảy nổ (Elastic Bounce)</option>
                    <option value="bounce_in">Nảy bóng bật cực phiêu (Bounce In)</option>
                    ...
                    <option value="stretch_in">Kéo dãn dẻo nảy (Elastic Stretch)</option>
                    <option value="fade_in">Smooth xuất hiện mờ (Fade In)</option>
                    <option value="typewriter">Đánh chữ gõ chạy (Typewriter)</option>
                    <option value="none">Hiện ngay tức khắc (None)</option>
                  </select>
                </div>
                ...
                {/* Exit movement */}
                <div>
                  <label className="text-[11px] text-white/55 block mb-1.5 font-bold uppercase tracking-wider">Hiệu ứng ra (Exit):</label>
                  <select
              ...
              <div className="border-t border-white/5 pt-4">
                <label className="text-[11px] text-sky-400 block mb-1.5 font-bold uppercase tracking-wider">Hiệu ứng chữ sống động (Active Show Effect):</label>
                <select
                ...
                >
                  ...
                  <option value="karaoke">🎤 Chữ chạy Karaoke (Đậm từng từ)</option>
                  <option value="tiktok_glow">🎵 Rung Neon Glow TikTok</option>
                  <option value="pulse_grow">💓 Nhịp thở phập phồng</option>
                  <option value="bounce_loop">🎈 Bay bổng dập dềnh liên tục</option>
                  ...
                  <option value="rainbow_flow">🌈 Sắc màu vồng cầu dải ngân hà</option>
                  <option value="glitch_cyber">👾 Nhiễu sóng Cyberpunk</option>
                </select>
              </div>

              {/* Position and alignment */}
              <div className="border-t border-white/5 pt-4 space-y-4">
                <span className="text-[11px] font-bold text-blue-400 block uppercase tracking-wider">Cấu hình chữ phụ đề (Text Options)</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full accent-blue-500 bg-[#050505] h-1 rounded cursor-pointer"
                    />
                  </div>
                ...
                {/* Custom alignment inside popup modal */}
                <div>
                  <span className="text-[11px] text-white/50 block mb-2 font-medium">Căn lề chữ mẫu:</span>
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { id: 'left', name: 'Căn Trái', lines: ['w-6', 'w-3', 'w-5', 'w-2'] },
                      { id: 'center', name: 'Căn Giữa', lines: ['w-5 mx-auto', 'w-3 mx-auto', 'w-6 mx-auto', 'w-2 mx-auto'] },
                      { id: 'right', name: 'Căn Phải', lines: ['w-6 ml-auto', 'w-3 ml-auto', 'w-5 ml-auto', 'w-2 ml-auto'] },
                      { id: 'justify', name: 'Căn Đều (Justify)', lines: ['w-full', 'w-full', 'w-full', 'w-full'] }
                    ].map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center gap-1.5 p-2 rounded-xl border transition-all text-[10px] font-bold ${
                            active
                              ? 'bg-blue-600/20 border-blue-500 text-blue-400'
                              : 'bg-[#030303] border-white/10 text-white/60 hover:text-white'
                          }`}
                        >
                          <div className="w-6 h-4 flex flex-col justify-between p-0.5 bg-black/40 border border-white/5 rounded">
                            {item.lines.map((lineWidth, index) => (
                              <div key={index} className={`h-[1px] rounded-full ${lineWidth} ${active ? 'bg-blue-400' : 'bg-white/30'}`} />
                            ))}
                          </div>
                          <span>{item.name}</span>
                        </button>
              ...
              {/* Independent blur options */}
              <div className="border-t border-white/5 pt-4 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[11px] font-bold text-sky-400 block uppercase tracking-wider">Cấu hình Độc lập Nền Mờ (Blur Glass Backdrop)</span>
                    <p className="text-[9px] text-white/40">Tự do chỉnh sửa vị trí kéo giãn, hình dáng cấu trúc, màu sắc dải mờ độc lập</p>
                  </div>
                  <button
                    type="button"
                ...
                {popupEditingEffect.enableBlurBg && (
                  <div className="space-y-4 animate-in slide-in-from-top duration-200">
                    {/* Lock text to blur background toggle */}
                    <div className="flex items-center justify-between bg-sky-500/5 border border-sky-500/10 rounded-xl p-2.5">
                      <div className="text-left">
                        <span className="text-[10px] font-bold text-sky-400 uppercase tracking-wide block">Khóa chữ nằm gọn trong nền mờ</span>
                        <span className="text-[8px] text-white/50 block mt-0.5">Tự động gắn bám chữ vào giữa nền & tự co giãn cỡ chữ để vừa vặn hoàn hảo</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setPopupEditingEffect((prev: any) => {
                          const nextLock = !prev.lockTextInBlur;
                          if (nextLock) {
                            return {
                              ...prev,
                        ...
                        className={`p-1 rounded transition-all flex items-center justify-center ${
                          popupEditingEffect.lockTextInBlur ? 'bg-sky-500/20 text-sky-400 border border-sky-500/30' : 'bg-black/40 text-white/20 border border-white/5 hover:border-white/10'
                        }`}
                      >
                        <input
                          type="checkbox"
                          ...
                          onChange={() => {}} 
                          className="accent-sky-500 w-3.5 h-3.5 rounded cursor-pointer pointer-events-none"
                        />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-1">
                          <span>Vị trí Ngang dải mờ (X):</span>
                        ...
                        <input
                          ...
                          className="w-full accent-sky-500 bg-[#050505] h-1 rounded cursor-pointer"
                        />
                      </div>

                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-1">
                          <span>Vị trí Dọc dải mờ (Y):</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-white/5 pt-3">
                      {/* Shape choice in popup */}
                      <div>
                        <label className="text-[11px] text-white/50 block mb-1">Hình dạng khối nền mờ (Shape):</label>
                        <select
                          value={popupEditingEffect.blurBgShape || 'rectangle'}
                          onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgShape: e.target.value }))}
                          className="w-full text-xs bg-[#050505] border border-white/10 rounded-lg px-2.5 py-1.5 text-white focus:outline-none focus:border-sky-500 font-semibold"
                        >
                          <option value="rectangle">Hình chữ nhật (Rectangle)</option>
                          <option value="rounded">Góc thu tròn (Rounded Rect)</option>
                          <option value="pill">Hình hạt đậu (Pill Capsule)</option>
                      ...
                      {/* Custom color hex choice in popup */}
                      <div>
                        <span className="text-[11px] text-white/50 block mb-1">Màu sắc sắc thái:</span>
                        <div className="flex items-center gap-2 bg-[#050505] p-1 border border-white/10 rounded-lg h-[34px]">
                          <input
                            type="color"
                            ...
                            className="w-6 h-6 rounded border border-white/10 bg-transparent cursor-pointer"
                          />
                          <span className="text-[10px] font-mono text-white/80">{(popupEditingEffect.blurBgColorHex || '#000000').toUpperCase()}</span>
                          
                          <div className="flex gap-1 ml-auto">
                            <button
                              type="button"
                              onClick={() => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgColorHex: '#000000' }))}
                              className="w-4 h-4 rounded-full bg-black border border-white/20"
                            />
                            <button
                              type="button"
                              onClick={() => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgColorHex: '#ffffff' }))}
                              className="w-4 h-4 rounded-full bg-white border border-white/20"
                            />
                          </div>
                        </div>
                      ...
                      {/* Custom border color choice in popup */}
                      <div>
                        <span className="text-[11px] text-white/50 block mb-1">Màu viền khối mờ (Border):</span>
                        <div className="flex items-center gap-2 bg-[#050505] p-1 border border-white/10 rounded-lg h-[34px]">
                          <input
                            type="checkbox"
                            checked={!!popupEditingEffect.blurBgBorderColorHex && popupEditingEffect.blurBgBorderColorHex !== 'none'}
                            onChange={(e) => {
                              const nextBorder = e.target.checked ? '#ffffff' : 'none';
                              setPopupEditingEffect((prev: any) => ({ ...prev, blurBgBorderColorHex: nextBorder }));
                            }}
                            className="w-4 h-4 accent-sky-500 rounded cursor-pointer ml-1"
                            title="Kích hoạt viền"
                          />
                          <input
                            type="color"
                            disabled={!popupEditingEffect.blurBgBorderColorHex || popupEditingEffect.blurBgBorderColorHex === 'none'}
                            value={popupEditingEffect.blurBgBorderColorHex && popupEditingEffect.blurBgBorderColorHex !== 'none' ? popupEditingEffect.blurBgBorderColorHex : '#ffffff'}
                            onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgBorderColorHex: e.target.value }))}
                            className="w-6 h-6 rounded border border-white/10 bg-transparent cursor-pointer disabled:opacity-40"
                          />
                          <span className="text-[10px] font-mono text-white/80 disabled:opacity-40">
                            {popupEditingEffect.blurBgBorderColorHex && popupEditingEffect.blurBgBorderColorHex !== 'none' ? popupEditingEffect.blurBgBorderColorHex.toUpperCase() : 'KHÔNG CÓ'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-3">
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-1">
                          <span>Độ trong suốt sắc tố:</span>
                          <span className="font-mono text-sky-400 font-bold">{Math.round((popupEditingEffect.blurBgOpacity !== undefined ? popupEditingEffect.blurBgOpacity : 0.5) * 100)}%</span>
                        </div>
                        <input
                          type="range"
                          min="0.1"
                          max="0.95"
                          step="0.05"
                          value={popupEditingEffect.blurBgOpacity !== undefined ? popupEditingEffect.blurBgOpacity : 0.5}
                          onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgOpacity: parseFloat(e.target.value) }))}
                          className="w-full accent-sky-500 bg-[#050505] h-1 rounded cursor-pointer"
                        />
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between text-[11px] text-white/50 mb-1">
                          <span>Tỉ lệ độ mờ nhám (px):</span>
                          <span className="font-mono text-sky-400 font-bold">{popupEditingEffect.blurBgBlurAmount !== undefined ? popupEditingEffect.blurBgBlurAmount : 18}px</span>
                        </div>
                        <input
                          type="range"
                          min="0"
                          max="40"
                          value={popupEditingEffect.blurBgBlurAmount !== undefined ? popupEditingEffect.blurBgBlurAmount : 18}
                          onChange={(e) => setPopupEditingEffect((prev: any) => ({ ...prev, blurBgBlurAmount: parseInt(e.target.value) }))}
                          className="w-full accent-sky-500 bg-[#050505] h-1 rounded cursor-pointer"
                        />
                      </div>
                    </div>
            ...
            {/* Footer actions */}
            <div className="p-4 border-t border-white/5 bg-[#09090c] flex items-center justify-end gap-3 [z-index:100]">
              <button
                type="button"
                onClick={() => setPopupEditingEffect(null)}
                className="px-4 py-2 border border-white/10 hover:border-white/25 rounded-md text-xs font-semibold text-white/70 hover:text-white transition-all active:scale-95"
              >
                Hủy bỏ
              </button>
              ...
              <button
                ...
                className="px-4 py-2 bg-zinc-800 hover:bg-zinc-750 text-white font-bold text-xs rounded-md transition-all active:scale-95 border border-white/10 flex items-center gap-1.5"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-blue-600 to-sky-600 hover:from-blue-500 hover:to-sky-555 text-white font-bold text-xs rounded-md shadow-lg shadow-blue-900/20 transition-all active:scale-95 flex items-center gap-1.5"
              >
                <Check size={12} /> Áp dụng & Lưu
              </button>
            </div>
